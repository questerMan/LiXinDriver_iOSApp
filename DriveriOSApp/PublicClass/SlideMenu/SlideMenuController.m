...
-(instancetype)init {
    ...
    if (self) {
        ...
        _hideStatusBar = YES;
        _pointOfNoReturnWidth = MATCHSIZE(200);
        _simultaneousGestureRecognizers = YES;
        _opacityViewBackgroundColor = [UIColor grayColor];
    }
    return self;
}
...
-(UIView *)leftPanView{
    if (!_leftPanView) {
        _leftPanView = [[UIView alloc] initWithFrame:CGRectMake(0, MATCHSIZE(0), MATCHSIZE(20), SCREEN_H)];
...
-(void)addLeftGestures {
    if (_leftViewController != nil) {
        ...
        if (self.leftPanGesture == nil) {
            ...
            //＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊
            [self.leftPanView addGestureRecognizer:self.leftPanGesture];
        }
...
#pragma mark - leftDidOpen打开左侧栏时移除左边小区域滑动手势＊＊＊＊＊＊＊＊＊＊
-(void)openLeftWithVelocity:(CGFloat)velocity {
    ...
    //打开左侧栏时移除左边小区域滑动手势
    self.leftPanView.frame = CGRectMake(-MATCHSIZE(20), MATCHSIZE(0), MATCHSIZE(20), SCREEN_H);
    [self.leftPanView removeGestureRecognizer:self.leftPanGesture];
    //把手势转移到view上
...
#pragma mark - leftDidClose自定义滑动宽度＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊
-(void)closeLeftWithVelocity:(CGFloat) velocity {
    ...
    [self.view removeGestureRecognizer:self.leftPanGesture];
    [self.leftPanView addGestureRecognizer:self.leftPanGesture];
    self.leftPanView.frame = CGRectMake(MATCHSIZE(0), MATCHSIZE(0), MATCHSIZE(20), SCREEN_H);
