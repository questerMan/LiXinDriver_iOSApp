...
-(void)initNavigationController{
    ...
    //右边专车按钮
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"car"] style:UIBarButtonItemStylePlain target:self action:@selector(rightItemOnclick:)];
}

-(void)rightItemOnclick:(UIBarButtonItem *)itemBtn{

    Indent *indent = [[Indent alloc] init];
...
#pragma mark - public 我的修改2 修改自己的头像/名称
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
            ...
            model.avatarImage = [UIImage imageNamed:@"EaseUIResource.bundle/user"];//头像
